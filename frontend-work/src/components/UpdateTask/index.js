...
const UpdateTask = ({ data, show, handleShowTaskModal, UpdatedTask, calledFrom }) => {
  ...
  useEffect(() => {
    if (allInternalUsers && fetchedCommentsData) {
      ...
      setTaskData({ ...taskData, assigned_users: assignedUsers, comments: fetchedCommentsData })
  ...
  const handleAddFiles = (type, e) => {
    ...
    console.log('surath 1', { ...taskData, [[type]]: [...taskData[[type]], ...totalfiles] })
    setTaskData({ ...taskData, [[type]]: [...taskData[[type]], ...totalfiles] })
  }
  ...
  const handleSaveTask = async () => {
    ...
    if (userData?.role == 'Admin') {
      taskData.assigned_users?.map((item) => {
        if (item?.uuid) {
          assignNewUsers.push(item)
        } else {
          const findUserIndex = deleteNotAssignUsers.findIndex((uuid) => uuid == item)
          if (findUserIndex != -1) {
        ...
        internalUserService
          ...
          .then((res) => {
            if (res.data.uuid == userData.uuid) {
              setUserData({ ...userData, assigned_tasks: [userData.assigned_tasks, taskData.uuid] })
            }
          })
      })
